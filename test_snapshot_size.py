...
from chastease.db import Base, build_engine, build_session_factory
...
try:
    ...
    # Use the actual snapshot builder function
    ...
    print("\n...\n")
    print(f"Snapshot tail (last 500 chars):")
    ...
    print(f"Current max_tokens for live requests: 2500")
    print(f"Estimated remaining tokens for AI response: ~{2500 - int(estimated_tokens)}")
    
    if 2500 - int(estimated_tokens) < 500:
        print("\n⚠️  WARNING: Less than 500 tokens remaining for AI response!")
        print(f"   Consider further reducing snapshot size or increasing max_tokens.")
    else:
